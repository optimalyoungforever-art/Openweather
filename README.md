...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .search-row { display:flex; gap:16px; flex:1; max-width:620px; position: relative; }
  ...
  /* Global/City Selector Datalist Container */
  .city-select-wrapper { position: absolute; top: 100%; left: 30px; right: 30px; max-height: 200px; background: rgba(6, 21, 40, 0.95); border: 2px solid var(--border); border-radius: 20px; margin-top: 8px; overflow-y: auto; z-index: 100; backdrop-filter: blur(12px); display: none; box-shadow: 0 10px 30px rgba(0,0,0,0.5); }
  .city-option { padding: 12px 20px; font-size: 16px; color: var(--text); cursor: pointer; transition: background 0.2s; }
  .city-option:hover { background: var(--glass2); color: var(--accent); }
...
<div class="app">
  ...
  <div class="header">
    ...
    <div class="search-row">
      <input type="text" id="city" value="Ruiru" placeholder="Search city or select from list..." autocomplete="off" oninput="filterCities(this.value)" onkeydown="if(event.key==='Enter') getWeather()"/>
      <div id="cityOptions" class="city-select-wrapper"></div>
...
// A broad list of cities from around the world to select from
const globalCities = [
  "Ruiru", "Nairobi", "Mombasa", "Kisumu", "New York", "London", "Tokyo", 
  "Paris", "Sydney", "Berlin", "Dubai", "Mumbai", "Toronto", "Singapore", 
  "Cape Town", "Cairo", "Beijing", "Seoul", "Moscow", "Rome", "Buenos Aires"
];
...
function filterCities(query) {
  ...
  const matches = globalCities.filter(c => c.toLowerCase().includes(query.toLowerCase()));
  if (matches.length > 0) {
    container.innerHTML = matches.map(c => `<div class="city-option" onclick="selectCity('${c}')">${c}</div>`).join('');
...
function getTempStyle(temp) {
  if (temp <= 10) return { color: '#81d4fa', glow: '#03a9f4', gradient: 'linear-gradient(90deg, #03a9f4, #81d4fa)' };
  if (temp <= 22) return { color: '#ffffff', glow: '#b0bec5', gradient: 'linear-gradient(90deg, #64b5f6, #b0bec5)' };
  if (temp <= 28) return { color: '#ffca28', glow: '#ffa000', gradient: 'linear-gradient(90deg, #ffca28, #ff7043)' };
  return { color: '#ef5350', glow: '#b71c1c', gradient: 'linear-gradient(90deg, #ff7043, #ef5350)' };
}
...
async function getWeather(){
  ...
  try{
    ...
    applyTheme('day');
    setState('ok');
    setWeatherElementColors(22, 84);

    document.getElementById('marqueeTicker').innerHTML = `
      <span style="color:var(--danger); margin-right:8px;">Network Warning:</span> 
      Using system backup diagnostics for ${city}.
    `;
...
// Automatically fetch weather for the default city on load
getWeather();
</script>
</body>
</html>
